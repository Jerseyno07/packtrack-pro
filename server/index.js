...
// with `credentials: true` meant an unset env var silently opened CORS to
// every origin in prod. Local dev (NODE_ENV !== 'production') keeps the
// permissive default so it still works without extra setup.
const corsOrigin = process.env.FRONTEND_ORIGIN || (process.env.NODE_ENV === 'production' ? false : true);
...
app.get('/api/v1/admin/overview', authenticate, requireRole('ADMIN'), asyncHandler(async (req, res) => {
  ...
  // Parameterized ($1), not string-interpolated — `since` is user-controlled
  // (req.query) and was previously spliced directly into the SQL text below,
  // which was a SQL injection vector reachable by any ADMIN-role account.
  const sinceParams = since ? [since] : [];
  const sinceDateClause = since ? 'AND indent_date >= $1' : '';
  const sincePoClause = since ? 'AND po_date >= $1' : '';
  const sinceIssuedClause = since ? 'AND issue_date >= $1' : '';
  const sinceReceiptClause = since ? 'AND receipt_date >= $1' : '';

  const [indents, pos, stock, issues, receipts, lowStock] = await Promise.all([
    pool.query(`SELECT il.*, w.name AS warehouse_name, m.code AS material_code, m.name AS material_name, m.unit FROM indent_lines il JOIN warehouses w ON w.id=il.warehouse_id JOIN materials m ON m.id=il.material_id WHERE 1=1 ${sinceDateClause} ORDER BY il.indent_date DESC`, sinceParams),
    pool.query(`SELECT po.*, m.code AS material_code, m.name AS material_name, m.unit, m.meters_per_unit, m.stickers_per_roll, m.pieces_per_kg, w.name AS warehouse_name FROM purchase_orders po JOIN materials m ON m.id=po.material_id JOIN warehouses w ON w.id=po.pm_store_warehouse_id WHERE 1=1 ${sincePoClause} ORDER BY po.po_date DESC`, sinceParams),
    ...
    pool.query(`SELECT si.*, m.code AS material_code, m.name AS material_name, fw.name AS from_warehouse_name, tw.name AS to_warehouse_name FROM stock_issues si JOIN materials m ON m.id=si.material_id JOIN warehouses fw ON fw.id=si.from_warehouse_id JOIN warehouses tw ON tw.id=si.to_warehouse_id WHERE 1=1 ${sinceIssuedClause} ORDER BY si.issue_date DESC`, sinceParams),
    pool.query(`SELECT sr.*, si.issue_ref, m.code AS material_code, m.name AS material_name FROM stock_receipts sr JOIN stock_issues si ON si.id=sr.stock_issue_id JOIN materials m ON m.id=si.material_id WHERE 1=1 ${sinceReceiptClause} ORDER BY sr.receipt_date DESC`, sinceParams),
...
app.get('/api/v1/stock/current', authenticate, asyncHandler(async (req, res) => {
  ...
  const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
  const result = await pool.query(
    `SELECT cs.warehouse_id, w.name AS warehouse_name, w.code AS warehouse_code, cs.material_id, m.code AS material_code, m.name AS material_name, m.unit,
            m.meters_per_unit, m.stickers_per_roll, m.pieces_per_kg,
...
//   fsn_id           → sku_code
//   weight_lot       → sku_name (not the "SKU Name" column — the vendor's real SKU-master
...
app.post('/api/v1/sku-packaging-master/upload', authenticate, requireRole('ADMIN'), upload.single('file'),
  asyncHandler(async (req, res) => {
    ...
    for (let i = 0; i < rawRows.length; i++) {
      ...
      validRows.push({
        sku_code, sku_name, source, primary_pm_code,
    ...
    for (let i = 0; i < dedupedRows.length; i += BATCH_SIZE) {
      ...
      await pool.query(
        `INSERT INTO sku_packaging_master (sku_code, sku_name, source, primary_pm_code, secondary_pm_code, tertiary_pm_code, uploaded_by, uploaded_at)
         SELECT sku_code, sku_name, source, primary_pm_code, secondary_pm_code, tertiary_pm_code, $7, now()
         FROM unnest($1::text[], $2::text[], $3::text[], $4::text[], $5::text[], $6::text[])
           AS t(sku_code, sku_name, source, primary_pm_code, secondary_pm_code, tertiary_pm_code)
