...
function LedgerSection({ token }) {
  ...
  const countIn  = rows.filter(r => Number(r.qty_delta) > 0).length;
  const countOut = rows.filter(r => Number(r.qty_delta) < 0).length;
  const countAdj = rows.filter(r => Number(r.qty_delta) === 0).length;
      ...
      {searched && !loading && rows.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-green-50 border border-green-100 rounded-xl p-4 flex items-center gap-3">
            ...
            <div>
              <div className="text-xs text-green-700 font-medium">Inward movements</div>
              <div className="text-lg font-bold text-green-800">{countIn}</div>
            </div>
          </div>
          <div className="bg-red-50 border border-red-100 rounded-xl p-4 flex items-center gap-3">
            <ArrowUpCircle size={20} className="text-red-600 flex-shrink-0" />
            <div>
              <div className="text-xs text-red-700 font-medium">Outward movements</div>
              <div className="text-lg font-bold text-red-800">{countOut}</div>
            </div>
          </div>
          <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 flex items-center gap-3">
            <BookOpen size={20} className="text-amber-600 flex-shrink-0" />
            <div>
              <div className="text-xs text-amber-700 font-medium">Total entries</div>
              <div className="text-lg font-bold text-amber-800">{rows.length}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {showTour && (
        <TourOverlay
          ...
          steps={[
            ...
            { target: 'sku-sample', title: 'Download SKU Sample', body: 'Download the sample to see the columns read: FSN ID (required), Weight Lot (stored as the SKU\'s name — not the SKU Name column), Source, EAN, Packing Type, Packing Material, plus optional secondary/tertiary columns for multi-material SKUs.' },
